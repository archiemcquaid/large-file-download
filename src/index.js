...
const downloadFile = (base64, filename, log = false) => new Promise((resolve, reject) => {
  try {
    if (!base64 || !filename) {
    ...
    const logger = log ? (stringToLog) => console.log(`large-file-downloader: ${stringToLog}`) : () => null;
    ...
    blobAnchor.addEventListener('click', () => {
      logger(`Downloading image ${filename}.`);
